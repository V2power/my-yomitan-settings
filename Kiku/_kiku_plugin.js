/**
 * @import { KikuPlugin } from "#/plugins/plugin-types";
 */

const fontsPool = [
  "'UD Digi Kyokasho NK', serif",
  "'Hiragino Kaku Gothic ProN', sans-serif",
  "'Aozora Mincho', serif",
  "'ackaisyo', sans-serif",
  "'MtyKjKyotai W4', serif",
  "'FOT Telop Min Pro', serif",
  "'Yu Kyokasho', serif",
];

// Fallback when storage is unavailable and the module survives a card flip.
let lastFont;
const fontLoads = new WeakMap();

/**
 * @type { KikuPlugin }
 */
export const plugin = {
  ExternalLinks: (props) => {
    const { html, useGeneralContext } = props.ctx;
    const { ankiDroidAPI } = useGeneralContext();

    function NadeshikoLink() {
      const url = new URL("https://nadeshiko.co/search/sentence");
      url.searchParams.set("query", props.ctx.$ankiFields.Expression);
      const src = `data:image/jpeg;base64,`;
      return html`<a href=${url.toString()}>
        <img class="size-5 object-contain rounded-xs" src=${src} />
      </a>`;
    }

    function LorenziLink() {
      const expression = props.ctx.$ankiFields.Expression || "";
      const url = `https://jisho.hlorenzi.com/search/${encodeURIComponent(expression)}`;
      const src = `_lorenzi_jisho_icon_round_256.png`;
      return html`<a href=${url}>
        <img class="size-5 object-contain rounded-xs" src=${src} />
      </a>`;
    }

    return [
      // includes the default ExternalLinks
      props.DefaultExternalLinks(),
      NadeshikoLink(),
      LorenziLink(),
      // you can create as many links as you want
    ];
  },
  onPluginLoad: ({ ctx }) => {
    const { $general } = ctx.useGeneralContext();
    const layout = $general.layoutRef;
    const { $initialSide } = ctx.useCardContext();
    let font;
    if ($initialSide() === "front") {
      font = fontsPool[Math.floor(Math.random() * fontsPool.length)];
      try {
        sessionStorage.setItem("random-font", font);
      } catch (_) {
        // Some WebView contexts may deny access to session storage.
      }
    } else {
      try {
        font = sessionStorage.getItem("random-font");
      } catch (_) {}
      font = font || lastFont || fontsPool[Math.floor(Math.random() * fontsPool.length)];
    }
    lastFont = font;
    if (!layout) return;
    layout.dataset.randomFontReady = "false";
    layout.style.setProperty("--font-secondary", font);

    // A cached/installed face can be revealed synchronously. Otherwise wait
    // for this face only, not every font in the document or a fixed timer.
    const token = {};
    fontLoads.set(layout, token);
    const reveal = () => {
      if (fontLoads.get(layout) === token) layout.dataset.randomFontReady = "true";
    };
    const fontSet = document.fonts;
    if (!fontSet || !fontSet.load) {
      reveal(); // Older WebViews rely on CSS font-display: block.
      return;
    }
    const query = (font === fontsPool[7] ? "700 " : "400 ") + "16px " + font;
    const text = layout.textContent || ctx.$ankiFields.Expression || " ";
    if (fontSet.check(query, text)) {
      reveal();
    } else {
      // A missing/corrupt font must not leave a study card permanently hidden.
      fontSet.load(query, text).then(reveal, reveal);
    }
  },
};
